...
use crate::{command::CommandEncoder, error::{AwsmError, Result}, renderer::AwsmRenderer, texture::TextureFormat};

impl AwsmRenderer {
    pub fn current_context_format(&self) -> TextureFormat {
        self.context.get_configuration().as_ref().unwrap().get_format()
    }

    pub fn current_context_texture(&self) -> Result<web_sys::GpuTexture> {
        self.context.get_current_texture().map_err(AwsmError::current_context_texture)
    }
    ...
    pub fn compile_shader(&self, shader_code: &web_sys::GpuShaderModuleDescriptor) -> web_sys::GpuShaderModule {
        self.device.create_shader_module(&shader_code)
    }

    pub async fn create_pipeline(&self, descriptor: &web_sys::GpuRenderPipelineDescriptor) -> Result<web_sys::GpuRenderPipeline> {
        let pipeline:web_sys::GpuRenderPipeline = JsFuture::from(self.device.create_render_pipeline_async(descriptor)).await
    ...
    pub fn create_pipeline_layout(&self, descriptor: &web_sys::GpuPipelineLayoutDescriptor) -> web_sys::GpuPipelineLayout {
        self.device.create_pipeline_layout(descriptor)
    }

    pub fn create_bind_group_layout(&self, descriptor: &web_sys::GpuBindGroupLayoutDescriptor) -> Result<web_sys::GpuBindGroupLayout> {
        self.device.create_bind_group_layout(descriptor).map_err(AwsmError::bind_group_layout)
    }

    pub fn create_bind_group(&self, descriptor: &web_sys::GpuBindGroupDescriptor) -> web_sys::GpuBindGroup {
        self.device.create_bind_group(descriptor)
    }

    pub fn create_sampler(&self, descriptor: Option<&web_sys::GpuSamplerDescriptor>) -> web_sys::GpuSampler {
        match descriptor {
    ...
    pub fn create_texture(&self, descriptor: &web_sys::GpuTextureDescriptor) -> Result<web_sys::GpuTexture> {
        self.device.create_texture(descriptor).map_err(AwsmError::texture_creation)
    }
    ...
    pub fn create_command_encoder(&self, label: Option<&str>) -> CommandEncoder {
        let encoder = match label {
            ...
            Some(label) => {
                ...
                self.device.create_command_encoder_with_descriptor(&descriptor)
            }
        };
