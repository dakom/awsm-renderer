...
use awsm_renderer_core::{bind_groups::{BindGroupDescriptor, BindGroupEntry, BindGroupLayoutResource, BindGroupResource, SamplerBindingLayout, SamplerBindingType, TextureBindingLayout}, renderer::AwsmRendererWebGpu, sampler::{FilterMode, SamplerDescriptor}, texture::{TextureSampleType, TextureViewDimension}};
use crate::{bind_group_layout::{BindGroupLayoutCacheKey, BindGroupLayoutCacheKeyEntry, BindGroupLayoutKey}, bind_groups::{AwsmBindGroupError, BindGroupRecreateContext}, error::Result, render_passes::{composite, RenderPassInitContext}, textures::{SamplerCacheKey, SamplerKey}};
...
impl DisplayBindGroups {
    ...
    pub fn get_bind_group(&self) -> std::result::Result<&web_sys::GpuBindGroup, AwsmBindGroupError> {
        self._bind_group.as_ref().ok_or_else(|| AwsmBindGroupError::NotFound("Display".to_string()))
    }
