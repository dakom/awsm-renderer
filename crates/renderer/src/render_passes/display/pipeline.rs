...
use crate::{error::Result, pipeline_layouts::{PipelineLayoutCacheKey, PipelineLayoutKey}, pipelines::render_pipeline::{RenderPipelineCacheKey, RenderPipelineKey}, render_passes::{display::{bind_group::DisplayBindGroups, shader::cache_key::ShaderCacheKeyDisplay}, RenderPassInitContext}};
...
impl DisplayPipelines {
    pub async fn new(ctx: &mut RenderPassInitContext, bind_groups: &DisplayBindGroups) -> Result<Self> {
        ...
        let render_pipeline_key = ctx.pipelines.render.get_key(&ctx.gpu, &ctx.shaders, &ctx.pipeline_layouts, render_pipeline_cache_key).await?;
