...
fn base_color_tex_uv(attribute_data_offset: u32, triangle_indices: vec3<u32>, barycentric: vec3<f32>, tex_info: TextureInfo, vertex_attribute_stride: u32) -> vec2<f32> {
    let uv0 = get_uv(attribute_data_offset, tex_info.attribute_uv_set_index, triangle_indices.x, vertex_attribute_stride);
    let uv1 = get_uv(attribute_data_offset, tex_info.attribute_uv_set_index, triangle_indices.y, vertex_attribute_stride);
    let uv2 = get_uv(attribute_data_offset, tex_info.attribute_uv_set_index, triangle_indices.z, vertex_attribute_stride);
