...
use crate::{bind_groups::{material::{MaterialBindGroupLayoutKey, MaterialBindingEntry, MaterialBindingLayoutEntry}, AwsmBindGroupError, BindGroups}, shaders::ShaderCacheKeyMaterial, textures::{SamplerKey, TextureKey, Textures}};
...
impl MaterialDeps {
    ...
    fn bind_group_layout_cache_key(&self) -> MaterialBindGroupLayoutCacheKey {
        match self {
            Self::Pbr(deps) => MaterialBindGroupLayoutCacheKey::Pbr(deps.bind_group_layout_cache_key())
        }
    }
...
impl Materials {
    ...
    pub fn get_or_insert(&mut self, gpu: &AwsmRendererWebGpu, bind_groups: &mut BindGroups, textures: &Textures, deps: MaterialDeps) -> Result<MaterialKey> {
        ...
        let bind_group_layout_key = match self.bind_group_layout_cache.get(&bind_group_layout_cache_key) {
            Some(key) => *key,
            None => {
        ...
        bind_groups.materials.insert_material(&gpu, material_key, bind_group_layout_key, &deps.bind_group_entries(textures)?).map_err(AwsmMaterialError::MaterialBindGroup)?;
