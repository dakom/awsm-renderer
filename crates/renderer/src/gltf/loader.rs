...
impl GltfResource {
    pub async fn load(url:&str, file_type: Option<GltfFileType>) -> anyhow::Result<Self> {
        ...
        Ok(Self { gltf: document, buffers, images })
    }
}
...
async fn import_buffer_data<'a>( document: &'a Document, base: &'a str, blob: Option<Vec<u8>>) -> anyhow::Result<Vec<Vec<u8>>> {
...
fn get_buffer_futures<'a>(document:&'a Document, base:&str, blob: Option<Vec<u8>>) -> Vec<impl Future<Output=anyhow::Result<Vec<u8>>> + 'a> {
        ...
        async move {
            match buffer.source() {
                ...
                buffer::Source::Bin => {
                    ...
                    blob.lock().unwrap().take().ok_or(GltfError::MissingBlob.into())
                }
            }
        } 
    }).collect()
...
async fn import_image_data<'a>(document: &'a Document, base: &'a str, buffer_data:&'a [Vec<u8>]) -> anyhow::Result<Vec<ImageLoader>> {
...
fn get_image_futures<'a>(document:&'a Document, base:&str, buffer_data:&'a [Vec<u8>]) -> Vec<impl Future<Output=anyhow::Result<ImageLoader>> + 'a> {
    //these need to be owned by each future simultaneously
    let base = Arc::new(base.to_owned());
