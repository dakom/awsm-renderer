...
use crate::{buffers::helpers::u8_to_f32_vec, gltf::{buffers::accessor_to_bytes, error::{AwsmGltfError, Result}}, AwsmRenderer};
    ...
    pub(super) fn populate_gltf_node_skin<'a, 'b: 'a, 'c: 'a>(
        ...
        if let Some(skin) = gltf_node.skin() {
            ...
            for joint_node in skin.joints() {
                let transform_key = *node_to_transform
                    .get(&joint_node.index())
                    .ok_or(AwsmGltfError::SkinJointTransformNotFound(joint_node.index()))?;
            ...
            let inverse_bind_matrices = match skin.inverse_bind_matrices() {
                Some(accessor) => {
                    ...
                    let matrices = values.chunks(16).map(|chunk| {
                        Mat4::from_cols_slice(chunk)
                    }).collect::<Vec<_>>();
            ...
            let _skin_key = self.skins.insert(joints, inverse_bind_matrices.unwrap_or_default());
        }
