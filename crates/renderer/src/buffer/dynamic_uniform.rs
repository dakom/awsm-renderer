...
mod test {
    ...
    fn test_update_existing_item() {
        ...
        assert_eq!(&buffer.raw_data[updated_offset..updated_offset + 16], b"updated data1234");
    }
    ...
    fn test_update_with_callback() {
        ...
        assert_eq!(&buffer.raw_data[offset + 4..offset + 8], &(offset as u32).to_le_bytes());
    }
    ...
    fn test_zero_value_variants() {
        ...
        let mut buffer1: DynamicUniformBuffer<TestKey, 0> = DynamicUniformBuffer::new(
            1, 8, Some(16), Some("zero_buffer".to_string())
        );
        
        // Test with custom zero value (0xFF)
        let mut buffer2: DynamicUniformBuffer<TestKey, 0xFF> = DynamicUniformBuffer::new(
            1, 8, Some(16), Some("ones_buffer".to_string())
        );
    ...
    fn test_resize_slot_allocation_correctness() {
        ...
        assert!(slot4 >= 3 && slot4 <= 5, "New key should use a slot from free_slots");
    ...
    fn test_resize_with_required_slots_exceeding_capacity() {
        ...
        println!("Before update: capacity_slots={}, next_slot={}, free_slots={:?}", 
        ...
        println!("After update: capacity_slots={}, next_slot={}, free_slots={:?}", 
                 buffer.capacity_slots, buffer.next_slot, buffer.free_slots);
        ...
        assert_eq!(free_slots_sorted, expected_free, 
                   "Free slots after resize are incorrect: {:?}", free_slots_sorted);
    ...
    fn test_resize_slot_range_allocation() {
        ...
        println!("After resize: capacity={}, next_slot={}, free_slots={:?}", 
                 buffer.capacity_slots, buffer.next_slot, buffer.free_slots);
        ...
        for (key, &slot) in buffer.slot_indices.iter() {
            assert!(!buffer.free_slots.contains(&slot), 
                    "Slot {} is both allocated to key {:?} and in free_slots", slot, key);
        }
    }
    ...
    fn test_resize_slot_assignment_consistency() {
        ...
        println!("Forced state: capacity={}, next_slot={}, free_slots={:?}", 
                 buffer.capacity_slots, buffer.next_slot, buffer.free_slots);
        ...
        println!("After update: capacity={}, next_slot={}, free_slots={:?}, slot_for_key1={:?}", 
                 buffer.capacity_slots, buffer.next_slot, buffer.free_slots, 
        ...
        assert!(!buffer.free_slots.contains(&3), "Allocated slot should not be in free_slots");
    ...
    fn test_resize_semantic_correctness() {
        ...
        buffer.remove(key1);
        let freed_slot = if buffer.slot_indices.get(key2) == Some(&0) { 1 } else { 0 };
        assert!(buffer.free_slots.contains(&freed_slot));
        
        println!("Before third allocation: capacity={}, next_slot={}, free_slots={:?}", 
                 buffer.capacity_slots, buffer.next_slot, buffer.free_slots);
    ...
    fn test_offset_consistency_after_resize() {
        ...
        println!("Before resize: key1->slot{} offset={}, key2->slot{} offset={}", 
        ...
        println!("After resize: key1->slot{} offset={}, key2->slot{} offset={}, key3->slot{} offset={}", 
        ...
        assert_eq!(offset1_before, offset1_after, "key1 offset changed during resize!");
        assert_eq!(offset2_before, offset2_after, "key2 offset changed during resize!");
        ...
        assert_eq!(offsets.len(), unique_offsets.len(), "Duplicate offsets detected!");
        
        // Verify data integrity after resize
        assert_eq!(&buffer.raw_data[offset1_after..offset1_after + 16], b"data1___________");
        assert_eq!(&buffer.raw_data[offset2_after..offset2_after + 16], b"data2___________");
        assert_eq!(&buffer.raw_data[offset3_after..offset3_after + 16], b"data3___________");
    }
    ...
    fn test_resize_next_slot_update() {
        ...
        let mut buffer: DynamicUniformBuffer<TestKey> = DynamicUniformBuffer::new(
            1, 16, Some(32), Some("next_slot_test".to_string())
        );
        let mut key_map = SlotMap::new();
        let mut keys = Vec::new();
        ...
        println!("Item 1: slot {}, next_slot={}, free_slots={:?}", 
                buffer.slot_indices.get(key1).unwrap(), buffer.next_slot, buffer.free_slots);
        
        // Add second item (forces resize to larger capacity)
        let key2 = key_map.insert(());  
        keys.push(key2);
        buffer.update(key2, b"ITEM_02_________");
        println!("Item 2: slot {}, next_slot={}, free_slots={:?}", 
                buffer.slot_indices.get(key2).unwrap(), buffer.next_slot, buffer.free_slots);
        
        // Verify that next_slot is beyond the free slots range to prevent conflicts
        assert!(buffer.next_slot > *buffer.free_slots.iter().max().unwrap_or(&0), 
        ...
        println!("Item 3: slot {}, next_slot={}, free_slots={:?}", 
                buffer.slot_indices.get(key3).unwrap(), buffer.next_slot, buffer.free_slots);
        
        let key4 = key_map.insert(());
        keys.push(key4);
        buffer.update(key4, b"ITEM_04_________");
        println!("Item 4: slot {}, next_slot={}, free_slots={:?}", 
                buffer.slot_indices.get(key4).unwrap(), buffer.next_slot, buffer.free_slots);
        ...
        println!("Item 5: slot {}, next_slot={}, free_slots={:?}", 
                buffer.slot_indices.get(key5).unwrap(), buffer.next_slot, buffer.free_slots);
        ...
        for (i, &key) in keys.iter().enumerate() {
            ...
            if !slots.insert(slot) {
                panic!("Duplicate slot detected: Item {} has slot {} which was already used", i+1, slot);
            }
            ...
            assert_eq!(data, expected.as_bytes(), "Data corruption detected for item {}", i+1);
        }
        
        println!("SUCCESS: All {} items have unique slots and intact data", keys.len());
        println!("Final slots used: {:?}", keys.iter().map(|k| buffer.slot_indices.get(*k).unwrap()).collect::<Vec<_>>());
    ...
    fn test_offset_after_multiple_resizes_and_removals() {
        ...
        let mut buffer: DynamicUniformBuffer<TestKey> = DynamicUniformBuffer::new(
            1, 16, Some(32), Some("offset_test".to_string())
        );
        let mut key_map = SlotMap::new();
        ...
        for i in 0..8 {
            ...
            assert_eq!(actual_offset, expected_offset, "Incorrect offset for key {} at iteration {}", i, i);
            ...
            for (j, (prev_key, prev_data)) in keys_and_data.iter().enumerate() {
                if let Some(prev_slot) = buffer.slot_indices.get(*prev_key) {
                    ...
                    assert_eq!(prev_actual_offset, prev_expected_offset, 
                              "Key {} offset corrupted at iteration {}", j, i);
                    
                    // Verify data is still intact
                    let data_slice = &buffer.raw_data[prev_actual_offset..prev_actual_offset + 16];
                    assert_eq!(data_slice, prev_data.as_bytes(), 
                              "Data corrupted for key {} at iteration {}. Expected: {:?}, Got: {:?}", 
                              j, i, prev_data.as_bytes(), data_slice);
                }
            }
        }
        
        println!("After adding 8 items: capacity={}, slots used: {:?}", 
                 buffer.capacity_slots, buffer.slot_indices.values().collect::<Vec<_>>());
        ...
        for (i, (key, data)) in keys_and_data.iter().enumerate() {
            if keys_to_remove.contains(key) {
                assert_eq!(buffer.offset(*key), None, "Removed key {} still has offset", i);
            } else {
                let slot = *buffer.slot_indices.get(*key).unwrap();
                let expected_offset = slot * 32;
                let actual_offset = buffer.offset(*key).unwrap();
                assert_eq!(actual_offset, expected_offset, "Offset wrong for remaining key {}", i);
                
                // Verify data is still intact
                let data_slice = &buffer.raw_data[actual_offset..actual_offset + 16];
                assert_eq!(data_slice, data.as_bytes(), "Data corrupted for remaining key {}", i);
            }
        }
        ...
        for i in 8..12 {
            ...
            let expected_offset = slot * 32;
            let actual_offset = buffer.offset(key).unwrap();
            assert_eq!(actual_offset, expected_offset, "Incorrect offset for new key {}", i);
            
            // Verify the data was written correctly
            let data_slice = &buffer.raw_data[actual_offset..actual_offset + 16];
            assert_eq!(data_slice, &data_bytes[..], "New item data not written correctly");
        }
    }
    ...
    fn test_offset_edge_cases_with_manual_state() {
        ...
        let mut buffer: DynamicUniformBuffer<TestKey> = DynamicUniformBuffer::new(
            3, 16, Some(64), Some("edge_offset_test".to_string()) // Different aligned_slice_size
        );
        let mut key_map = SlotMap::new();
        ...
        unique_offsets.sort();
        unique_offsets.dedup();
        assert_eq!(all_offsets.len(), unique_offsets.len(), "Offset collision detected!");
    }
    ...
    fn test_offset_boundary_values() {
        ...
        let mut buffer: DynamicUniformBuffer<TestKey> = DynamicUniformBuffer::new(
            1, 1, Some(1), Some("boundary_offset_test".to_string()) // Minimal sizes
        );
        let mut key_map = SlotMap::new();
    ...
    fn test_no_dangling_slots_after_resize() {
        ...
        let mut buffer: DynamicUniformBuffer<TestKey> = DynamicUniformBuffer::new(
            2, 16, Some(32), Some("slot_accessibility_test".to_string())
        );
        let mut key_map = SlotMap::new();
        ...
        println!("After first resize: capacity={}, next_slot={}, free_slots={:?}", 
                buffer.capacity_slots, buffer.next_slot, buffer.free_slots);
        ...
        for i in 4..7 { // Allocate slots 3,4,5
            let key = key_map.insert(());
            all_keys.push(key);
        ...
        println!("After using free slots: free_slots={:?}, next_slot={}", 
        ...
        println!("After second resize: capacity={}, next_slot={}, allocated_slot={:?}", 
                buffer.capacity_slots, buffer.next_slot, buffer.slot_indices.get(key7));
        ...
        println!("After extensive allocation: max_slot_used={}, total_keys={}", 
                max_slot_seen, buffer.slot_indices.len());
        ...
        println!("Slot usage efficiency: {:.1}% ({} keys using slots 0-{})", 
                efficiency * 100.0, total_keys, max_slot_seen);
        
        // We should have reasonably efficient slot usage (> 50% is acceptable given the safety tradeoff)
        assert!(efficiency > 0.5, "Slot usage efficiency should be reasonable: {:.1}%", efficiency * 100.0);
