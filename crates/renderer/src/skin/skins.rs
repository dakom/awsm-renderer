...
use super::error::{Result, AwsmSkinError};
...
impl Skins {
    ...
    pub fn insert(&mut self, joints: Vec<TransformKey>, inverse_bind_matrices: Vec<Mat4>) -> Result<SkinKey> {
        for (index, joint) in joints.iter().enumerate() {
            // check if the inverse bind matrix has diverged
            match (self.inverse_bind_matrices.get(*joint), inverse_bind_matrices.get(index)) {
            ...
            self.inverse_bind_matrices.insert(*joint, inverse_bind_matrices.get(index).cloned().unwrap_or(Mat4::IDENTITY));
        }
        let offset = self.buffer_bytes.len();
        self.buffer_bytes.extend_from_slice(&vec![0u8; joints.len() * 16 * 4]);
    ...
    pub fn update_and_write_gpu(&mut self, gpu: &AwsmRendererWebGpu, world_matrices: &SecondaryMap<TransformKey, Mat4>) -> Result<()> {
        ...
        for (skin_key, skin_joints) in self.joints.iter() {
            ...
            for (skin_joint_index, skin_joint) in skin_joints.iter().enumerate() {
                ...
                let world_matrix = match self.inverse_bind_matrices.get(*skin_joint).cloned() {
                    Some(inverse_bind_matrix) => {
                        world_matrices.get(*skin_joint).map(|m| *m * inverse_bind_matrix).unwrap()
                    },
                    None => world_matrices.get(*skin_joint).cloned().unwrap(),
                };

                // just overwrite this one matrix
                let bytes = unsafe { std::slice::from_raw_parts(world_matrix.as_ref().as_ptr() as *const u8, 16 * 4) };
